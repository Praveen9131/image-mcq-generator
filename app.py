...
def describe_image(image_url: str):
    image_prompt = [
        ...
        {"role": "user", "content": f"Describe the content of the following image URL: {image_url}. Please provide a detailed and accurate description."}
    ]
...
def generate_image_options(option_prompts: list):
    options = []
    for prompt in option_prompts:
        ...
        if image_url:
            ...
            # Provide a fallback option if image generation fails
            fallback_url = f"https://dummyimage.com/270x140/000/fff.png&text=Image+Not+Available"
            options.append(fallback_url)
    return options

# Function to generate a question with image options based on the image description
...
def generate_content():
    try:
        ...
        for _ in range(num_questions):
            image_prompt = f"An educational illustration representing the topic: {topic}. The illustration should be clear and informative and more clear."
